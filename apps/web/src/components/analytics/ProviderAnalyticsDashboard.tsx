...
export function ProviderAnalyticsDashboard() {
  ...
  const closed =
    (values?.outcomes.WIN ?? 0) + (values?.outcomes.LOSS ?? 0) + (values?.outcomes.BREAKEVEN ?? 0);
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {analytics.isError ? (
        <ErrorState
          message="Provider analytics could not be loaded."
        ...
        <>
          <dl className="mb-8 grid gap-px overflow-hidden rounded border border-border bg-border sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Metric
              icon={TrendingUp}
              label="Average R:R"
              value={`${values.avgRiskReward.toFixed(2)}R`}
            />
            <Metric
              icon={BarChart3}
              label="Average PnL"
              value={`${values.avgPnlPercent.toFixed(2)}%`}
            />
          </dl>
          <section className="mb-8">
            <div className="mb-4">
              <h2 className="text-sm font-medium">Verified outcome distribution</h2>
              <p className="mt-1 text-xs text-text-muted">
                Based on {values.verifiedOutcomeCount} market-verified or administrator-corrected
                outcomes.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <Outcome
                label="Breakeven"
                value={values.outcomes.BREAKEVEN ?? 0}
                color="bg-warning"
              />
            </div>
          </section>
      ...
      <section>
        ...
        {signals.isError ? (
          <ErrorState
            message="Recent signals could not be loaded."
            retry={() => void signals.refetch()}
          />
        ) : null}
        {signals.data?.data.length === 0 ? (
          <EmptyState title="Publish a signal to populate provider analytics." />
        ) : null}
...
function Metric({ icon: Icon, label, value }: { icon: typeof Zap; label: string; value: string }) {
  return (
    <div className="bg-background-surface p-5">
      ...
      <dt className="mt-4 text-xs text-text-muted">{label}</dt>
...
function Outcome({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="rounded border border-border bg-background-surface p-4">
      <span className={`mb-3 block h-1 w-10 rounded ${color}`} />
      <p className="text-xs text-text-muted">{label}</p>
      <p className="numeric mt-2 text-2xl">{value}</p>
    </div>
  );
}
